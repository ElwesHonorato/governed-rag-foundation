...
class ChunkTextProcessor:
    ...
    def build_input_dataframe(self, processed: dict[str, Any], *, doc_id: str) -> Any:
        """Create Spark DataFrame input for chunk transforms."""
        ...
        return self.spark_session.createDataFrame([base_row])

    def write_chunk_artifacts_from_dataframe(self, input_df: Any) -> tuple[int, list[str]]:
        """Transform Spark DataFrame rows into chunk artifacts and write outputs."""
        records = self._build_chunk_records_spark(input_df)
        written = 0
        destination_keys: list[str] = []
        for chunk_record in records:
