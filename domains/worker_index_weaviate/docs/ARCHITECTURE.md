...
worker_index_weaviate is a queue-driven stage worker in the pipeline.

Problem it solves:
- Upserts embeddings into Weaviate and writes indexed-status artifacts.
...
Boundary:
- Input: Index queue messages and embedding objects
- Output: Weaviate upserts and indexed-status objects
...
Design:
- Composition root in `src/worker_index_weaviate/app.py`.
...
- src/worker_index_weaviate/app.py
...
6. queue settlement policy:
   - valid + success: `ack()`
   - invalid payload: publish `index_weaviate.invalid_message` to DLQ, then `ack()`
   - processing failure: publish `index_weaviate.failure` to DLQ, then `ack()`
