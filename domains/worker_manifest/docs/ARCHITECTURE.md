...
worker_manifest is a queue-driven stage worker in the pipeline.

Problem it solves:
- Computes and writes per-document manifest status across pipeline stages.
...
Boundary:
- Input: Processed/chunks/embeddings/indexes storage prefixes
- Output: Manifest status artifacts
