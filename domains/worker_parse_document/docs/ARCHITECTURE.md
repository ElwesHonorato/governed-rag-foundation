...
worker_parse_document is a queue-driven stage worker in the pipeline.

Problem it solves:
- Parses raw documents into processed JSON payloads for downstream chunking.
...
Boundary:
- Input: Parse queue messages and raw-stage objects
- Output: Processed objects and chunk queue messages
...
6. queue settlement policy:
   - valid + success: `ack()`
   - invalid payload: publish `parse_document.invalid_message` to DLQ, then `ack()`
   - handling/processing failure: `nack(requeue=True)` so message can retry
