...
worker_scan is a queue-driven stage worker in the pipeline.

Problem it solves:
- Scans incoming storage objects, copies them to raw stage, and enqueues parse jobs.
...
Boundary:
- Input: Incoming storage prefix objects
- Output: Raw stage objects and parse queue messages
...
Design:
- Composition root in `src/worker_scan/app.py`.
