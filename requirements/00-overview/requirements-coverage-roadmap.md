...
|---|---|---|---|---|
| FR-01 | Unified ingestion across unstructured/structured/streaming/APIs/DB | Partial | `domains/worker_scan/app/src/worker_scan/app.py`, `domains/worker_parse_document/app/src/worker_parse_document/app.py` | Current implementation covers S3-driven document flow only; SharePoint/SAP/Oracle/Kafka/Flink/API/DB connectors are still missing. |
...
| A-01 | Full governed RAG flow (ingest->chunk->mask->embed->hybrid retrieve->grounded response) | Missing | `domains/worker_*/app/src`, `apps/rag-api/src/rag_api/services/prompt_service.py` | Worker stages exist, but masking and governed retrieval/grounded response behavior are still incomplete. |
| A-02 | Context-aware chunking, parent-child indexing, versioned indexing | Partial | `domains/worker_chunk_text/app/src/worker_chunk_text/app.py`, `domains/worker_index_weaviate/app/src/worker_index_weaviate/app.py` | Chunking and indexing stages exist, but advanced context-aware and versioning features are incomplete. |
| INT-01 | S3 + SharePoint document sources | Partial | S3 config in `.env.example`; S3 usage in `domains/worker_*/app/src` | S3 pipeline is present; SharePoint integration remains missing. |
...
| AC-01 | Ingestion coverage for all source classes | Missing | `domains/worker_scan/app/src/worker_scan/app.py` | Current ingestion remains limited to S3-driven processing. |
